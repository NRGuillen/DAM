...
int main(){
	...
	if (strcmp(respuesta, "si") == 0){ 
		printf("Ingrese el nombre del producto que quieres retirar: ");
		scanf(" %s", nombre2);
	...
	int productoNuevo = 0; 
		for (int i = 0; i < num; i++){
			...
			printf("Cantidad a quitar: "); //Pedimos la cantidad nueva
			scanf(" %d", &cantidadnueva[i]);//Guardamos en la cantidad a sumar
			//"COMENTARIO EXAMEN
			//se resta la cantidad nueva con la anterior dejandonos la resta
