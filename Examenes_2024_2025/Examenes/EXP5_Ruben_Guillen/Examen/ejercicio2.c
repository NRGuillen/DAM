...
int main (){
	...
	//declaro variables para comprobar g y e	    
	int tieneE;
	int tieneG;
	
	//Bucle para recorrer las 40 palabras de mi lista, "char palabras"
	...
	// se cambiara a 1, indicando que la palabra tiene una 'a'.
		...
		//Inicializo a 1 e g para cuando tengan una palabra cambie a 0
		tieneE = 1;
		tieneG = 1;
	...
	// El bucle se detiene cuando "i > 7", es decir cuando haya llegado ejecutado "DAM"
		...
		for (int comprobar = 0; palabrasE[i][comprobar]; comprobar++){
			if(palabrasE[i][comprobar] == 'e' || palabrasE[i][comprobar] == 'E'){
				tieneE = 0;}
			
		
			else if(palabrasG[i][comprobar] == 'g' || palabrasG[i][comprobar] == 'G'){
			 	tieneG = 0;}
		}
